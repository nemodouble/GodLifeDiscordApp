...
class UICog(commands.Cog):
    """UI 관련 명령 및 모달/버튼 처리를 위한 코그(테스트 스텁)"""
    ...
    async def routine(self, interaction: discord.Interaction):
        ...
        # 채널에 패널 전송
        await interaction.response.send_message("루틴 패널입니다. 버튼을 사용하세요.", view=MainPanelView(), ephemeral=False)

    async def process_add_routine(self, itx: discord.Interaction, data: dict):
        print("process_add_routine 호출 by", itx.user, data)
        # DB에 루틴 생성
        try:
            rid = await routine_repo.create_routine(str(itx.user.id), data.get("name"), data.get("weekend_mode"), data.get("deadline_time"), data.get("notes"))
